...
struct Position {
    ...
    {
        size_t operator()(const Position& s) const 
        {
            size_t h1 = std::hash<int>{}(s.row);
            size_t h2 = std::hash<int>{}(s.col);
...
public:
    ...
    {
        const static std::string text[] = {"#REF!", "#VALUE!", "#DIV/0!"};
...
public:
    ...
    struct CellInterPtrHasher
    {
        size_t operator()(const CellInterface* ptr)
        {
