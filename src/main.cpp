...
void TestPrintTable()
{
    ...
    sheet->SetCell("A2"_pos, "8");
    sheet->SetCell("A3"_pos, "10");
    sheet->SetCell("A4"_pos, "2b");

    try
    {
        sheet->SetCell("A1"_pos, "=A2 +// A7");
        ASSERT(false);
    }
    catch (const FormulaException&) {}

    sheet->SetCell("B1"_pos, "=(A1 + A2) / A3");
    sheet->SetCell("B2"_pos, "=(A1 + A2) / A16387");
    sheet->SetCell("B3"_pos, "=(A1 + A2) / 0");
    sheet->SetCell("B4"_pos, "=(A1 + A2) / A4");
